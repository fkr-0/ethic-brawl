...
export interface E2EProbeSnapshot {
  ...
  renderer: {
    ...
    bridgeContextState: 'ready' | 'lost' | null;
    bridgePasses: string[];
...
export function updateE2EStatus(snapshot: E2EProbeSnapshot): void {
  ...
  element.dataset.rendererBridgeRequested = String(snapshot.renderer.bridgeRequested);
  element.dataset.rendererBridgeFailure = snapshot.renderer.bridgeFailureReason ?? '';
  element.dataset.rendererBridgeContext = snapshot.renderer.bridgeContextState ?? 'none';
  element.dataset.rendererBridgePasses = snapshot.renderer.bridgePasses.join(',');
